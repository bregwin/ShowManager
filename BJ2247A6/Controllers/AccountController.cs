    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(string returnUrl)
        {
            if (UserManager.Users.Count() == 0)
            {
                ...
                if (result.Succeeded)
                {
                    await UserManager.AddClaimAsync(adminUser.Id, new Claim(ClaimTypes.Email, "admin@example.com"));
                    await UserManager.AddClaimAsync(adminUser.Id, new Claim(ClaimTypes.Role, "Administrator"));
                    await UserManager.AddClaimAsync(adminUser.Id, new Claim(ClaimTypes.GivenName, "Application"));
                    await UserManager.AddClaimAsync(adminUser.Id, new Claim(ClaimTypes.Surname, "Administrator"));
                }
